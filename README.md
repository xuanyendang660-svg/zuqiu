...
如需复盘赛前冻结的唯一比分，使用新增的 `football-score` 实验入口。它训练完整联合比分分布并按最大概率格选一个比分，比较历史 Dixon–Coles 基线，按赛前可核实的报价时间决定是否启用市场分支，并对冻结稿逐场审计方向、分差、败方进球、总球带和停表状态。详细数据合同、运行方法和时间外回测见 [docs/exact_score_research.md](docs/exact_score_research.md)。

这套研究入口与上述 `football-value` 投注入口分开：旧比分分支的人工常见比分惩罚和尾部奖励不用于新模型。现有历史保留期里联合模型未胜过 Dixon–Coles，对缺少报价时间戳的市场不声称已通过双基线验证。
