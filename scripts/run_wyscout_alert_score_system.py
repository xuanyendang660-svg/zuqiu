...
from football_v2.live_alert_score_system import (
    leave_one_league_out_alert_score_test,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Live tail-alert plus exact-score test"
    )
    parser.add_argument("--data-root", required=True)
    parser.add_argument("--market-cache", default=".cache/football-data")
    parser.add_argument("--workers", type=int, default=8)
    parser.add_argument(
        "--output", default="artifacts/wyscout_alert_score.json"
    )
    ...
    report = leave_one_league_out_alert_score_test(live_dataset)
    payload = {
        "alert_score_system": report.to_dict(),
        "dataset": {
