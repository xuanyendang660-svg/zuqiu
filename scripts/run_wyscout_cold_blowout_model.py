...
from football_v2.live_cold_blowout_model import (
    leave_one_league_out_cold_blowout_model_test,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Direct live cold-blowout cluster model"
    )
    parser.add_argument("--data-root", required=True)
    parser.add_argument("--market-cache", default=".cache/football-data")
    parser.add_argument("--workers", type=int, default=8)
    parser.add_argument(
        "--output", default="artifacts/wyscout_cold_blowout_model.json"
    )
    ...
    report = leave_one_league_out_cold_blowout_model_test(live_dataset)
    payload = {
        "cold_blowout_model": report.to_dict(),
        "dataset": {
