...
from football_v2.live_cold_blowout_cluster import (
    leave_one_league_out_cold_blowout_test,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Cold blowout cluster detection and score ranking"
    )
    parser.add_argument("--data-root", required=True)
    parser.add_argument("--market-cache", default=".cache/football-data")
    parser.add_argument("--workers", type=int, default=8)
    parser.add_argument(
        "--output", default="artifacts/wyscout_cold_blowout_cluster.json"
    )
    ...
    report = leave_one_league_out_cold_blowout_test(live_dataset)
    payload = {
        "cold_blowout_cluster": report.to_dict(),
        "dataset": {
