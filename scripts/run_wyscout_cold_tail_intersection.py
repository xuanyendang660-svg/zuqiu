...
from football_v2.live_cold_tail_intersection import (
    leave_one_league_out_cold_tail_test,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Live cold-tail intersection test"
    )
    parser.add_argument("--data-root", required=True)
    parser.add_argument("--market-cache", default=".cache/football-data")
    parser.add_argument("--workers", type=int, default=8)
    parser.add_argument(
        "--output", default="artifacts/wyscout_cold_tail_intersection.json"
    )
    ...
    report = leave_one_league_out_cold_tail_test(live_dataset)
    payload = {
        "cold_tail_intersection": report.to_dict(),
        "dataset": {
