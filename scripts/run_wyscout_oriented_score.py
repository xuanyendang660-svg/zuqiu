...
from football_v2.live_oriented_score import (
    leave_one_league_out_oriented_score_test,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Underdog/favorite oriented remaining-score test"
    )
    parser.add_argument("--data-root", required=True)
    parser.add_argument("--market-cache", default=".cache/football-data")
    parser.add_argument("--workers", type=int, default=8)
    parser.add_argument(
        "--output", default="artifacts/wyscout_oriented_score.json"
    )
    ...
    report = leave_one_league_out_oriented_score_test(live_dataset)
    payload = {
        "oriented_remaining_score": report.to_dict(),
        "dataset": {
            "matches": int(live_dataset.frame["match_id"].nunique()),
            "snapshots": len(live_dataset.frame),
            "cold_tail_rate": float(
