...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Nested cross-league cold-blowout ranking test"
    )
    parser.add_argument("--data-root", required=True)
    parser.add_argument("--market-cache", default=".cache/football-data")
    parser.add_argument("--workers", type=int, default=8)
    parser.add_argument(
        "--output", default="artifacts/wyscout_cold_blowout_ranker.json"
    )
    ...
    report = leave_one_league_out_cold_blowout_ranker_test(
        live_dataset,
        cutoff=30,
        budgets=(0.02, 0.04, 0.06),
    )
    payload = {
        "cold_blowout_ranker": report.to_dict(),
        "dataset": {
