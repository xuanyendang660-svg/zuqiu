...
from football_v2.live_remaining_goals import (
    leave_one_league_out_remaining_goal_test,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Conditional remaining-goals exact-score test"
    )
    parser.add_argument("--data-root", required=True)
    parser.add_argument("--market-cache", default=".cache/football-data")
    parser.add_argument("--workers", type=int, default=8)
    parser.add_argument(
        "--output", default="artifacts/wyscout_remaining_goals.json"
    )
